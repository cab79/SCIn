...
switch opt
    ...
    % settings options
    h.SettingsOptions = {'Ascend','Adaptive','Assoc'};
    ...
    h.Settings.wait=[0.3 1.7]; % within-trial frequency (Hz); one value per nstim 
    
    %% first stimulus: colour dot
    % duration of stimulus in seconds
    h.Settings.stim(1).dur = h.Settings.wait(1); % modified by oddball settings
    ...
    h.Settings.stim(1).dotColor = [1 0 0];
    h.Settings.stim(1).dotSizePix = 100;
    
    %% first stimulus: fixation dot
    % duration of stimulus in seconds
    h.Settings.stim(2).dur = h.Settings.wait(2); % modified by oddball settings
    ...
    h.Settings.stim(2).dotColor = [1 1 1];
    h.Settings.stim(2).dotSizePix = 10;
