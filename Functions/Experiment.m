...
while h.i<size(h.Seq.signal,2)
                ...
                if ~isnan(h.Seq.adapttype(h.i+1))
                    if isfield(h,'s') 
                        ...
                        if isfield(h.Settings.adaptive_general,'terminate')
                            if ~isfield(h.s,'terminate')
                                h.s.terminate=[];
                            end
                            if strcmp(h.Settings.adaptive_general.terminate,'block')
                                if h.i==1 || h.Seq.blocks(h.i)>h.Seq.blocks(h.i-1)
                                    h.s.terminate = [];
                                end
                            end
                        end
                        
                        if ismember(h.Seq.adapttype(h.i),h.s.terminate)
    ...
    if isfield(h.Settings,'response_nexttrialwait') && ~isempty(h.Settings.response_nexttrialwait)
...
%end
    ...
    % exit if nexttrial
    if isfield(h,'nexttrialtime') && ~isempty(h.nexttrialtime)
        ...
        if isfield(h.Settings,'buttontype')
            if ~isempty(h.Settings.buttontype)
                if h.Settings.record_response
                    ...
                    % quit PTB Screen
                    if h.pressed
                        if strcmp(KbName(firstPress),'ESCAPE')
                            ShowCursor;
                            sca;
                            h.quit=1;
                            return
